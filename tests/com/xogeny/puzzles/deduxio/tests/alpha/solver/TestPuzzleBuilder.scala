...
class TestPuzzleBuilder extends FunSuite {
  ...
  test("Test uniqueness") {
    ...
    (0 to 100) foreach { seed =>
      ...
      val cons = builder.craft();
      if (verbose) println("Selected constraints: "+cons);
      builder.baseConstraints foreach { c: Constraint => assert(c.consistent(prob.board, sol) )}
      if (verbose) println("Base constraints are consistent");
      cons foreach { c: Constraint => assert(builder.baseConstraints.contains(c)) }
      if (verbose) println("Selected constraints are among base constraints")
      val incon = cons filter { !_.consistent(prob.board, sol); }
      if (verbose) println("Solution: "+sol);
      if (verbose) println("Inconsistent constraints: "+incon);
      cons foreach { c: Constraint => assert(c.consistent(prob.board, sol)) }
      if (verbose) println("Chosen constraints are consistent with solution")
      ...
      builder.baseConstraints.foldLeft(SetSolver.forProblem(prob)) { (s, c) =>
        val ns = s.solve().size;
        if (verbose) println("Current # of solution: "+ns);
        assert(ns>0)
        if (verbose) println("imposing "+c);
        s.impose(c);
      }
      if (verbose) println("Using base constraints, bcsolver.vals is correct");
      assert(bcsolver.solve().size==1);
      if (verbose) println("Using base constraints, we get exactly one solution");
      val solver: SetSolver = cons.foldLeft(SetSolver.forProblem(prob)) { (s, c) => s.impose(c) }
      if (verbose) println("solver.vals = "+solver.vals);
      val ssol = solver.solve();
      if (verbose) println("Generated puzzle had "+ssol.size+" solutions");
      assert(cons.length>0)
      assert(ssol.size==1);
      assert(ssol==Set(sol));
    }
